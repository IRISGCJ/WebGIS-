...
def parse_table_data(data):
    ...
    for row in data:
        ...
        if len(cols) >= 4:
            ...
            location_href = cols[2].xpath('.//a/@href')[0]
            location_url = f"https://en.wikipedia.org{location_href}"
            current_stadium = cols[2].xpath('string()').strip()
...
# 爬取和显示数据
url = 'https://en.wikipedia.org/wiki/2023%E2%80%9324_Premier_League#Teams'
...
df1.to_csv('Premier League 2023-2024.csv')
